...
function createRecord(number) {
  ...
  session
    .run('MATCH (you:User {name: {name}}) UNWIND {friends}  as name  MATCH (other:CONSTELLATION {name:name}) CREATE (you)-[:CONSTELLATION_OWNED]->(other)', 
    { 
      name: `jacky${number}`, 
      friends: getRandomArray('CONSTELLATION', 1, 1, 12)
    })
    .subscribe({
      onNext: (record) => {
        console.log(number);
