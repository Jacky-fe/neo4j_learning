...
function createRecord(number) {
  ...
  session
    .run('MATCH (you:User {name: {name}}) UNWIND {friends}  as name  MATCH (other:Game {name:name}) CREATE (you)-[:PLAY_GAME {weight:{weight}}]->(other)', 
    { 
      name: `jacky${number}`, 
      friends: getRandomArray('Game', getRandomInt(1, 10), 1, 20),
